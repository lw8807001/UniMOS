...
def load_image(image_filename, evaluating=False, label_filename=None): #导入图片
    """
    """
    image = nib.load(image_filename) #导入图片

    if evaluating and path.isfile(label_filename): #导入标签
        label = nib.load(label_filename)
    else:
        label = None

    return {'image':image, 'label':label} #返回图片和标签的字典

# %%

def load_network(fn_network, gpu=True): #导入模型参数
    ...
    if path.isfile(fn_network):
        ...
        #model = ResUNet(3,4)
        model = ResUNet(3, 2) #参数：输入通道，输出通道（种类数（含背景））
...
def compute_dice(la, lb): #计算Dice
    intersection = np.sum(la * lb) #计算“交”的部分
    union = np.sum(la + lb) #计算“并”的部分
    return 2 * intersection / (union + 0.00001) #计算Dice，+0.00001防止除0
...
if __name__ == "__main__":
    ...
    fn_network = path.join(args.network_path,'resu_best_' + view + '.pth.tar') #模型参数路径
    ...
    model_axial, optimizer = load_network(fn_network, gpu=use_cuda) #导入参数后的模型
    # Set model to evaluation mode
    model_axial.eval() #评估模式

    #img_filename = path.expanduser(args.input_filename)
    #file in computer/home/data/ct_nih
    img_filename = args.input_filename #图片路径
    print('Input image for segmentation:\t{}'.format(img_filename))

    dicom_input = False #导入的图片是否为dcm格式

    # Check if it is DICOM folder
    if path.isdir(img_filename): #传入的图片路径是目录（dcm格式？）
        ...
        volume = load_image(img_filename, evaluating, args.label_filename) #返回的包含图片和标签的字典

        image, label = volume['image'], volume['label'] #图片和标签
        #print('image_type = ', type(image)) #<class 'nibabel.nifti1.Nifti1Image'>
        #print('image_shape = ', image.shape) #(256, 256, 463)
        w, h, d = image.shape[:3] #宽 高 深
        #print('w = {} h = {} d = {}'.format(w, h, d)) #w = 256 h = 256 d = 463

        img_data = np.squeeze(image.get_data()) #图片数据
        #print('image_getdata_type = ', type(image.get_data())) #<class 'numpy.memmap'>
        #print('img_data_type = ', type(img_data)) #<class 'numpy.memmap'>
        #print('img_data_shape = ', img_data.shape) #(256, 256, 463)
        

    print('Size of the input image: {}x{}x{}'.format(w, h, d))

    img_data = img_data.astype(np.float32) #改变图片数据的类型

    if view == 'axial':
        img_data = img_data # W H D
    elif view == 'coronal':
            img_data = img_data.transpose((2,0,1)) # D W H
    else:
        img_data = img_data.transpose(2,1,0) # D H W

    img_data[img_data > 200] = 200.0 #超200的部分置为200
    img_data[img_data < -200] = -200.0 #小于-200的部分置为-200
    img_data /= 200.0 #将图片数据固定到[-1, 1]范围内
    ...
    num_segmented_slices = last - first + 1 #分割的层数
    print('Segmenting {} slices between [{}, {}]'.format(
        num_segmented_slices, first, last)) #分割[first, last-1]通道的图像

    for i in range(first): #[0, first-1]的结果用0代替
        #results.append(np.zeros((1,1,w,h)))
        results.append(np.zeros((1,h,w)))
    print('[{}, {}]'.format(0, first - 1))
    #for depth in range(d - num_channels + 1):
    print('[{}, {}]'.format(first - num_half_channels, last - num_half_channels - 1))
    for depth in range(first - num_half_channels,
        ...
        if dicom_input:
            ...
            subvolume = img_data[:,:,depth:depth+num_channels]
            subvolume = subvolume.transpose((2, 1, 0)) # D H W
        ...
        for volume in subvolumes:
            ...
            #print('output5_unique = ', output5.unique())
            # output_s = nn.Softmax2d()(output5)
            outputs.append(output5)
        
        output = construct_volume(outputs, x_coor, y_coor)
        #print('output1_unique = ', output.unique())
        output = output.max(dim=0)[1].cpu().data.numpy()
        #print('output2_unique = ', torch.tensor(output).unique())
        output = output[np.newaxis,:,:]
        #print('output3_unique = ', torch.tensor(output).unique())
    ...
    for i in range(d - last):
        #results.append(np.zeros((1,1,w,h)))
        results.append(np.zeros((1,h,w)))
    print('[{}, {}]'.format(last - num_half_channels, last - num_half_channels + d - last - 1))
    for i in range(last - num_half_channels + d - last, 511):
        results.append(np.zeros((1,h,w)))
    print('[{}, {}]'.format(last - num_half_channels + d - last, 512))
    ...
    results_post = np.zeros_like(results) #生成和results尺寸相同的全零矩阵
    print('results_unique = ', torch.tensor(results).unique())
    min_co = 0
    for i in range(1, 4):
        #liver
        if i ==1:
            ...
            results_i[results == i] = 1 #将results中属于当前类别的对应像素设为1
